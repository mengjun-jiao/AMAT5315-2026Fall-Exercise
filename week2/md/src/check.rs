...
pub fn evaluate(meta:&RunMetadata,frames:&[Frame])->Result<CheckReport,String> {
    let model=crate::physics::PhysicalModel::PeriodicShiftedLennardJones{box_size:meta.box_size,rc:2.5};
    let mut totals=Vec::with_capacity(frames.len());let mut speeds=Vec::new();
    for (index,f) in frames.iter().enumerate() {
        ...
        for (name,saved,recomputed) in [("E_pot",f.e_pot,u),("E_kin",f.e_kin,k)] {
            if !crate::trajectory::close(saved,recomputed) {
                return Err(format!("traj.jsonl line {}: {name} mismatch: saved={saved:.15e}, recomputed={recomputed:.15e}",index+1));
            }
        }
        totals.push(u+k);
...
fn metrics(energies:&[f64],speeds:&[f64])->Result<CheckReport,String> {
    if energies.is_empty() || speeds.is_empty() || energies[0]==0. || !energies.iter().all(|e|e.is_finite()) || !speeds.iter().all(|v|v.is_finite() && *v>=0.) {
    ...
    if !t_speed.is_finite() || t_speed<=0. || !drift.is_finite() { return Err("nonpositive/nonfinite T_speed or nonfinite drift".into()); }
    let edges:Vec<f64>=(1..24).map(|k|(-2.*t_speed*(1.-k as f64/24.).ln()).sqrt()).collect();
    let mut counts=[0usize;24];
    for &v in speeds { counts[edges.partition_point(|&b|v>=b)]+=1; }
    let expected=speeds.len() as f64/24.;
    let mb_score=counts.iter().map(|&o|(o as f64-expected).powi(2)/expected).sum::<f64>()/22.;
    let passed=drift<2e-3 && (t_speed-0.5).abs()<0.05 && mb_score<2.;
    Ok(CheckReport{drift,t_speed,mb_score,passed})
}
#[cfg(test)]
mod tests {
    use super::*;
