...
#[cfg(test)]
mod periodic_cache_tests {
    use super::*;
    #[test]
    fn periodic_verlet_refreshes_once_per_step() {
        let mut system = crate::fluid::initialize(&crate::fluid::RunConfig::default()).unwrap();
        assert_eq!(system.force_evaluations, 1);
        for completed in 1..=5 {
            VelocityVerlet.step(&mut system, 0.01);
            assert_eq!(system.force_evaluations, 1 + completed);
        }
    }
}
