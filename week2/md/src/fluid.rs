...
impl RunConfig {
    pub fn validate(&self)->Result<(),String> {
        if [self.rho,self.temperature,self.dt].iter().any(|x|!x.is_finite() || *x<=0.) {
            return Err("rho, temperature and dt must be finite and positive".into());
        }
        if self.sample_every==0 || self.steps<self.sample_every || !(self.steps as f64*self.dt).is_finite() {
            return Err("steps/sample_every must produce at least one finite-time frame".into());
        }
        if self.integrator!="velocity-verlet" && self.integrator!="euler" { return Err("unknown integrator".into()); }
...
pub fn lattice(n:usize,rho:f64)->Result<(Vec<[f64;2]>,[f64;2]),String> {
    let m=n.isqrt();
    if m<2 || m.checked_mul(m)!=Some(n) || m%2!=0 { return Err("n must be a square with an even number of rows".into()); }
    if !rho.is_finite() || rho<=0. { return Err("positive finite rho required".into()); }
    let a=(2./(3_f64.sqrt()*rho)).sqrt();let h=3_f64.sqrt()*a/2.;
    let box_size=[m as f64*a,m as f64*h];
    crate::physics::PhysicalModel::PeriodicShiftedLennardJones{box_size,rc:2.5}.validate()?;
    let mut p=Vec::with_capacity(n);
    for j in 0..m { for i in 0..m { p.push([(i as f64+0.5*(j%2) as f64)*a,j as f64*h]); } }
    Ok((p,box_size))
}
pub fn initialize(c:&RunConfig)->Result<System,String> {
    ...
    for v in &mut vel { for x in v { let z:f64=StandardNormal.sample(&mut rng);*x=z*c.temperature.sqrt(); } }
    for axis in 0..2 {
        let mean=vel.iter().map(|v|v[axis]).sum::<f64>()/c.n as f64;
        for v in &mut vel { v[axis]-=mean; }
    }
    let mut s=System::with_model(pos,vel,crate::physics::PhysicalModel::PeriodicShiftedLennardJones{box_size,rc:2.5})?;
