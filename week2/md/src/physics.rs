...
impl PhysicalModel {
    pub fn validate(&self) -> Result<(), String> {
        if let Self::PeriodicShiftedLennardJones { box_size, rc } = self {
            if !rc.is_finite()
                || *rc <= 0.0
                || box_size.iter().any(|l| !l.is_finite() || *l <= 2.0 * rc)
            {
    ...
    pub fn displacement(&self, a: [f64; 2], b: [f64; 2]) -> [f64; 2] {
        ...
        if let Self::PeriodicShiftedLennardJones { box_size, .. } = self {
            for axis in 0..2 {
                d[axis] -= box_size[axis] * (d[axis] / box_size[axis]).round();
            }
        }
        d
    }
    ...
    pub fn pair(&self, r: f64) -> Result<(f64, f64), String> {
        if !r.is_finite() || r <= 0.0 {
            return Err("nonzero finite separation required".into());
        }
        let u = match self {
        ...
        let f = crate::force(r);
        if !u.is_finite() || !f.is_finite() {
            return Err("nonfinite pair energy or force".into());
        }
        Ok((u, f))
    }
}
