...
fn execute(cli: Cli) -> Result<(), String> {
    match cli.command {
        ...
        Some(Commands::Check { directory }) => {
            ...
            println!(
                "T_speed={:.15e}; abs(T_speed-0.5)={:.15e} < 0.05 {}",
                r.t_speed,
                (r.t_speed - 0.5).abs(),
                label((r.t_speed - 0.5).abs() < 0.05)
            );
            println!(
                "chi2/22={:.15e} < 2 {} (teaching tolerance, not a significance test)",
                r.mb_score,
                label(r.mb_score < 2.)
            );
            if !r.passed {
                return Err("physical acceptance failed".into());
            }
            println!("PASS");
        }
