...
fn checked(command: &mut Command, label: &str) -> Result<std::process::Output, String> {
    ...
    if !out.status.success() {
        return Err(format!("{label}: {}", String::from_utf8_lossy(&out.stderr)));
    }
    Ok(out)
}
pub fn render_video(dir: &Path, out: &Path) -> Result<(), String> {
    ...
    checked(
        Command::new(&python)
            ...
            .args(["-c", "import numpy, matplotlib"]),
        "Python video dependencies (set MD_PYTHON)",
    )?;
    ...
    if !String::from_utf8_lossy(&encoders.stdout).contains("libx264") {
        return Err("ffmpeg requires libx264".into());
    }
    ...
    let mut writer =
        std::io::BufWriter::new(std::fs::File::create(&data).map_err(|e| e.to_string())?);
    for f in &frames {
    ...
    let movie = tempfile::Builder::new()
        .suffix(".mp4")
        .tempfile_in(parent)
    ...
    let count = String::from_utf8_lossy(&probe.stdout)
        .trim()
        .parse::<usize>()
        .map_err(|e| format!("invalid video frame count: {e}"))?;
    let bytes = movie.as_file().metadata().map_err(|e| e.to_string())?.len();
    if count != frames.len() || bytes >= 2_000_000 || bytes == 0 {
        return Err(format!(
