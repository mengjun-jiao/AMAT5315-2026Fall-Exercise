...
pub fn read_trajectory(dir:&Path)->Result<(RunMetadata,Vec<Frame>),String> {
    ...
    let meta:RunMetadata=serde_json::from_reader(std::fs::File::open(&path).map_err(|e|format!("{}: {e}",path.display()))?)
        .map_err(|e|format!("run.json: {e}"))?;
    meta.config.validate().map_err(|e|format!("run.json: {e}"))?;
    let (_,expected_box)=crate::fluid::lattice(meta.n,meta.rho)?;
    for axis in 0..2 {
        if !close(meta.box_size[axis],expected_box[axis]) { return Err("run.json: box/rho/n mismatch".into()); }
    }
    let model=crate::physics::PhysicalModel::PeriodicShiftedLennardJones{box_size:meta.box_size,rc:2.5};model.validate()?;
    let path=dir.join("traj.jsonl");
    let input=BufReader::new(std::fs::File::open(&path).map_err(|e|format!("{}: {e}",path.display()))?);
    let mut frames=Vec::new();let expected_count=meta.steps/meta.sample_every;
    for (index,line) in input.lines().enumerate() {
        ...
        if index>=expected_count { return Err(format!("{context}: excess frame")); }
        let expected_step=(index+1).checked_mul(meta.sample_every).ok_or("step overflow")?;
        if f.step!=expected_step || !close(f.t,expected_step as f64*meta.dt) { return Err(format!("{context}: step/time mismatch")); }
        if f.pos.len()!=meta.n || f.vel.len()!=meta.n { return Err(format!("{context}: pos/vel length mismatch")); }
        if !f.pos.iter().chain(&f.vel).flatten().all(|x|x.is_finite()) || !f.e_pot.is_finite() || !f.e_kin.is_finite() {
            return Err(format!("{context}: nonfinite state or energy"));
        }
        for p in &f.pos { for axis in 0..2 { if p[axis]<0. || p[axis]>=meta.box_size[axis] { return Err(format!("{context}: pos outside box")); } } }
        crate::physics::energies(&f.pos,&f.vel,&model).map_err(|e|format!("{context}: {e}"))?;
        frames.push(f);
    }
    if frames.len()!=expected_count { return Err(format!("traj.jsonl: frame count {}, expected {expected_count}",frames.len())); }
    Ok((meta,frames))
}
pub(crate) fn close(actual:f64,expected:f64)->bool {
    actual.is_finite() && expected.is_finite() && (actual-expected).abs()<=1e-10*expected.abs().max(1.)
}
pub fn run_to_directory(c:&RunConfig,dir:&Path)->Result<(),String> {
    ...
    let meta=RunMetadata{config:c.clone(),box_size};
    ...
    traj_file.persist(dir.join("traj.jsonl")).map_err(|e|e.to_string())?;
    meta_file.persist(dir.join("run.json")).map_err(|e|e.to_string())?;
    Ok(())
}
