...
fn model()->PhysicalModel { PhysicalModel::PeriodicShiftedLennardJones { box_size:[8.,6.], rc:2.5 } }
#[test]
...
fn pair_energy_counted_once_and_total_internal_force_zero() {
    ...
    assert!((u+md::energy(2.5)).abs()<1e-12); assert_eq!(k,5.);
...
fn rejects_invalid_models_states_and_periodic_coincidence() {
    assert!(PhysicalModel::PeriodicShiftedLennardJones{box_size:[5.,6.],rc:2.5}.validate().is_err());
