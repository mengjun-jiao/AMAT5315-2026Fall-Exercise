...
fn density_normalization_counts_beyond_force_cutoff() {
    ...
    assert!((s.values[3]-expected).abs()<1e-12);assert_eq!(s.frames,1);
    assert_eq!(s.centers,vec![0.5,1.5,2.5,3.5,4.5]);
    let again=w.push(&[[0.,0.],[3.,0.]]).unwrap();assert_eq!(again.values,s.values);
}
#[test]
fn twenty_first_frame_evicts_first() {
    ...
    assert_eq!(last.frames,20);assert_eq!(last.values[1],0.);
...
fn periodic_distance_and_maximum_radius_endpoint() {
    ...
    assert!(w.push(&[[0.,0.],[6.,0.]]).unwrap().values.iter().all(|g|*g==0.));
}
